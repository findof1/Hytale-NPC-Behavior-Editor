...
{
  if (!targetNode)
    return General::Action::Action{General::Action::Type::Nothing};

  QString type = targetNode->nodeType();
  if (serializer->actionSerializers.contains(type))
  {
    General::Action::Action actionData = serializer->actionSerializers[type](targetNode);

    if (type == "Sequence" && actionData.type == General::Action::Type::Sequence)
    {
      General::Action::Sequence &sequenceActionData = static_cast<General::Action::Sequence &>(actionData);
      ...
      for (auto &link : scene->links)
      {
        ...
        {
          sequenceActionData.actions.actions.emplace_back(serializeAction(serializer, scene, targetNode2));
        }
      }
    }
    ...
    {
      General::Action::Timeout &timeoutActionData = static_cast<General::Action::Timeout &>(actionData);
...
{
  if (!targetNode)
    return General::Sensor::Sensor{General::Sensor::Type::Any};

  QString type = targetNode->nodeType();

  if (serializer->sensorSerializers.contains(type))
  {
    General::Sensor::Sensor sensorData = serializer->sensorSerializers[type](targetNode);

    if (type == "And" && sensorData.type == General::Sensor::Type::And)
    {
      General::Sensor::And &andSensorData = static_cast<General::Sensor::And &>(sensorData);
      ...
      for (auto &link : scene->links)
      {
        ...
        QString socketName = link.startNode->getName();
        if (socketName == "Sensors")
        {
          andSensorData.sensors.emplace_back(serializeSensor(serializer, scene, targetNode2));
        }
      }
    }

    if (type == "Or" && sensorData.type == General::Sensor::Type::Or)
    {
      General::Sensor::Or &orSensorData = static_cast<General::Sensor::Or &>(sensorData);
      ...
      for (auto &link : scene->links)
      {
        ...
        if (socketName == "Sensors")
        {
          orSensorData.sensors.emplace_back(serializeSensor(serializer, scene, targetNode2));
        }
      }
    }

    if (type == "BlockType" && sensorData.type == General::Sensor::Type::BlockType)
    {
      General::Sensor::BlockType &blockTypeSensorData = static_cast<General::Sensor::BlockType &>(sensorData);
    ...
    if (type == "Not" && sensorData.type == General::Sensor::Type::Not)
    {
      General::Sensor::Not &notSensorData = static_cast<General::Sensor::Not &>(sensorData);
    ...
    if (type == "ValueProviderWrapper" && sensorData.type == General::Sensor::Type::ValueProviderWrapper)
    {
      General::Sensor::ValueProviderWrapper &valueProviderWrapperSensorData = static_cast<General::Sensor::ValueProviderWrapper &>(sensorData);
...
std::vector<General::Instruction> serializeInstructions(SerializerValues *serializer, NodeScene *scene, NodeItem *startNode)
{
  ...
  for (auto &rootLink : scene->links)
  {
    ...
    for (auto &link : scene->links)
    {
      if (link.startNode->parentItem() != instrNode)
        continue;
